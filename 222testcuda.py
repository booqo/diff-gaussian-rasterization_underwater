...
def gradient_check(variable_name, variable, rasterizer, **kwargs):
    """
    检查指定变量的梯度是否正确。
    ...
    autograd_grad_flat = autograd_grad.view(-1)




# c_med_plusd = c_med
# sigma_bs_plusd = sigma_bs
# sigma_atten_plusd = sigma_atten
# colors_enhance_plusd = colors_enhance

gradient_check(
    variable_name='medium_rgb',
    variable=c_med_plusd,  # 要检查的变量
    ...
    rotations=rotations,
    cov3D_precomp=None,
    medium_rgb=c_med_plusd,
    medium_bs=sigma_bs_plusd,
    medium_attn=sigma_atten_plusd,
    colors_enhance=colors_enhance_plusd
)


# desired_colors_plusd = np.array([
#     [100, 0.0, 0.0],   # 高斯体 1: 红色
#     # [0.0, 100, 0.0],   # 高斯体 2: 绿色
...
#
# # 执行高斯光栅化
# color_attn_plusd , color_clr_plusd, color_medium_plusd, radii_plusd, depths_plusd = rasterizer(
#     means3D=pts,               # 高斯体的3D位置
#     means2D=screenspace_points,  # 高斯体在屏幕空间的2D位置
#     shs=shs,                   # 球谐颜色
#     colors_precomp=None,       # 预计算颜色（如果有）
...
#     rotations=rotations,       # 旋转矩阵
#     cov3D_precomp=None,        # 预计算协方差（如果有）
#     medium_rgb=c_med_plusd,          # 介质颜色
#     medium_bs=sigma_bs_plusd,        # 介质散射系数
#     medium_attn=sigma_atten_plusd,   # 介质衰减系数
#     colors_enhance = colors_enhance_plusd # 增强的颜色
# )
...
'''----------------------------------------------------------'''
# d_attn = color_attn_plusd-color_attn
# d_clr =  color_clr_plusd-color_clr
# d_medium = color_medium_plusd-color_medium
# d_radii = radii_plusd-radii
# d_depth = depths_plusd- depths

# d_attn_np = d_attn.detach().cpu().numpy()
# d_clr_np = d_clr.detach().cpu().numpy()
# d_medium_np = d_medium.detach().cpu().numpy()
# d_radii_np = d_radii.detach().cpu().numpy()
# d_depth_np = d_depth.detach().cpu().numpy()
